...
public class ViewKidsClothsFragment extends Fragment {
    private RecyclerView clothListRv;
    private String paymentId = "";
    public static final int KIDS_CLOTHS = 2;

    public ViewKidsClothsFragment(String paymentId) {
    ...
    private void setAdapter(List<UpdatePriceResponce> clothList) {
        ...
        ClothsCountAdapter adapter = new ClothsCountAdapter(clothList, getContext(), KIDS_CLOTHS);
    ...
    private void filterAdultCloths(List<UpdatePriceResponce> clothList) {
        ...
        for (int i = 0; i < clothList.size(); ) {
            if (clothList.get(i).getWearId() != KIDS_CLOTHS) {
                clothList.remove(i);
            } else {
                i++;
            }
